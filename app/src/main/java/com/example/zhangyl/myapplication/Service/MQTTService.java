...
import com.example.zhangyl.myapplication.Presenter.FaceRecognitionMsg;
...
public class MQTTService extends Service {
    ...
    private void onFaceRecognitionTopic(String strMsg){
        //todo 独立成接口类，交给P层实现
        Log.i(TAG, "onFaceRecognitionTopic: event bus post face topic msg");
        EventBus.getDefault().post(new FaceRecognitionMsg(strMsg));
    }
    ...
    private MqttCallback mqttCallback = new MqttCallback() {
        ...
        public void messageArrived(String topic, MqttMessage message) throws Exception {
            // todo 按topic分发给不同的监听器，触发不同的业务线，各业务线异步执行任务。
            ...
            Log.i(TAG, str2);
            if(topic.equals("face_recognition")){
                onFaceRecognitionTopic(str1);
            }
        }
