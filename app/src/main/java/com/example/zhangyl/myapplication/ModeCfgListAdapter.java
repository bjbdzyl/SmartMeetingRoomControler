...
public class ModeCfgListAdapter extends CommonAdapter<MeetingModeCfgFragment.ListViewItem>{

    private int nMode;

    public ModeCfgListAdapter(Context context, int layoutId, List<MeetingModeCfgFragment.ListViewItem> datas) {
        super(context, layoutId, datas);
    }

    @Override
    protected void convert(final ViewHolder viewHolder, final MeetingModeCfgFragment.ListViewItem item, int position) {
        ...
        on_off_btn.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                item.modeCfg.setCfg(nMode, isChecked, seek_bar.getProgress());
                EventBus.getDefault().post(new ModeCfgChangeEvent(item.modeCfg));
            }
        });
    }
